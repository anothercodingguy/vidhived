...
def analyze_document_task(document_id: str, gcs_uri: str):
    ...
    try:
        ...
        for clause in clauses:
            ...
            # Only call Gemini for important clauses to save costs
            if category in ["Red", "Yellow"]:
                prompt = f"""
                You are a smart legal co-pilot named Vidhived.ai. Your goal is to explain legal text in plain English for non-lawyers.
                Analyze the following clause from a legal document.
                
                Document Clause: "{clause_text}"

                Tasks:
                1. Identify the type of clause (e.g., "Termination Clause", "Notice Period", "Confidentiality", "Payment Terms").
                2. Explain what this clause means in simple, easy-to-understand terms. Focus on the practical impact for the user.
...
# --- Error handlers and healthcheck ---
@app.errorhandler(400)
def handle_bad_request(error) -> Tuple[dict, int]:
    return jsonify({"error": "Bad Request", "details": getattr(error, "description", None)}), 400
